...
public class EnvironmentSpecificPropertiesResolver implements PropertiesResolver {
    ...
    private static final Logger log = LoggerFactory.getLogger(EnvironmentSpecificPropertiesResolver.class);
    ...
    public EnvironmentSpecificPropertiesResolver(PropertiesResolverBuilder builder) {
        ...
        this.resolvingEnvironmentVariables = builder.isResolvingEnvrionmentVariables();
        this.resolvingSystemProperties = builder.isResolvingSystemProperties();
        this.applicationName = builder.getApplicationName();

        ResourceLoader opsOverride =   builder.getOperationalOverridesResourceLoader();
        if(opsOverride == PropertiesResolverBuilder.DEFAULT_RESOURCE_LOADER_FOR_OPERATION_OVERRIDES) {
            if(applicationName!=null && applicationName.trim().length()>0) {
                opsOverride = new FileSystemResourceLoader(PropertiesResolverBuilder.DEFAULT_OPERATIONAL_OVERRIDE_LOCATION + applicationName + PropertiesResolverBuilder.DEFAULT_CONFIGURATION_LOCATION);
            }
        }
...
//        SERVER_ENV.SERVER_TYPE.TARGET_ENV
    ...
    public boolean isTrimmingPropertyValues() {
        return trimmingPropertyValues;  //To change body of implemented methods use File | Settings | File Templates.
    }

    @Override
    public boolean isResolvingSystemProperties() {
        return resolvingSystemProperties;  //To change body of implemented methods use File | Settings | File Templates.
    }

    @Override
    public boolean isResolvingEnvironmentVariables() {
        return resolvingEnvironmentVariables;  //To change body of implemented methods use File | Settings | File Templates.
    }

    @Override
    public String getProperty(String simplePropertyName) {
        Properties mergedProperties = _getMergedProperties();
        return mergedProperties.getProperty(simplePropertyName);
    }
